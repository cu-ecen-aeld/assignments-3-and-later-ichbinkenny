...
void* single_list_get_node_data(struct list_node* entry)
{
    return entry->data;
}

void single_list_delete_node(struct list_node_head* list_head, struct list_node* target)
{
    // Find the parent and child of the node.
    struct list_node* parent = list_head->next;
    struct list_node* child = target->next;

    // Clean up the target node's data. We're destructively deleting it so we should free it.
    free(target->data);
