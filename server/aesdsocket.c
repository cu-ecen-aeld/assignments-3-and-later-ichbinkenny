...
void *handle_client(void *client_ptr)
{
  ...
  while ((num_read_bytes = recv(client.socket, string_data + current_position, MAX_START_BUFFER_SIZE, 0)) > 0)
  {
...
void run_server(int server_socket)
{
  ...
  while (!should_terminate)
  {
                                      ...
                                      &client_addr_len)))
    {
      ...
      for (struct list_node *entry = thread_list_head.next; entry != NULL; entry = entry->next)
      {
